    ...
    for (const auto& sf_value: sfList) {
        ...
        for (const auto& cf_value: cfList) {
            ...
            if (workingMode == EchoProbeWorkingMode::Injector)
                LoggingService_Plugin_info_print("EchoProbe injector %s @ cf=%.3fMHz, sf=%.3fMHz, #.tx=%d.", nic->getReferredInterfaceName(), (double) cf_value / 1e6, (double) sf_value / 1e6, tx_count);
            else if (workingMode == EchoProbeWorkingMode::EchoProbeInitiator)
                LoggingService_Plugin_info_print("EchoProbe initiator %s @ cf=%.3fMHz, sf=%.3fMHz, #.tx=%d, #.acked=%d, echo_delay=%.1fms, success_rate=%.1f%%.", nic->getReferredInterfaceName(), (double) cf_value / 1e6, (double) sf_value / 1e6, tx_count, acked_count, mean_delay_single, double(100.0 * acked_count / tx_count));
        }
