...
void EchoProbePlugin::parseAndExecuteCommands(const std::string& commandString) {
    ...
    if (vm.count("target-mac-address")) {
        ...
        if (eachHexs.size() != 6)
            LoggingService_Plugin_warning_print("[target-mac-address] Specified mac address has wrong number of digits.\n");
        else {
    ...
    if (vm.count("sf")) {
        ...
        if (nic->getDeviceType() == PicoScenesDeviceType::IWL5300) {
            LoggingService_Plugin_warning_print("Intel 5300 NIC does not support sampling rate configuration.\n");
